...
Azure TTS の HTTP エラーでは `AzureTtsError` がスローされ、`status`、`statusText`、`responseBody`、`requestId` から Azure の診断情報を確認できます。Playground のサーバー側ログにもこれらの情報とリージョン、SSML の文字数が出力されます。サブスクリプションキーや SSML 本文自体はログに出力されません。
